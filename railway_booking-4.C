...
Node* start = NULL;
void add_node(char a[], char b[], int);
void details(int);
int seat(int);
int cal(int, int, int);
void bill(int, int);
...
void main()
{
	...
	// and amount argument
	bill(d, j);
	printf("Do you want to see past bookings? (yes/no)");
	scanf("%s", choice);
...
void bill(int y, int j)
{
	...
	int i;
	Node* ptr = start;
	fp = fopen("past_bookings.txt", "a");
	fprintf(fp, "Transcation details:\n");
	for (i = 1; i <= j; i++) {
